...
const Home1 = () => {
  ...
  useEffect(() => {
    const fetchAgencies = async () => {
      ...
      try {
        ...
        if (!userId || !accessToken) throw new Error('ไม่พบข้อมูลการเข้าสู่ระบบ');
  ...
  const handleSubMenuItemClick = (mainTabName, subItemName) => {
    setActiveSubTabs(prev => ({ ...prev, [mainTabName]: subItemName }));
    setOpenSubMenu(null);
    navigate("/home", { 
      state: { 
